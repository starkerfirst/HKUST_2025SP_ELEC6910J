...
def epsilon_greedy_policy(nS, nA, Q_function, eps=0.5):
    ...
    for state in range(nS):
        ...
        policy[state, :] = eps / nA
        policy[state][best_action] += 1 - eps
...
def sample_action(policy, state):
    ...
    prob = policy[state]
    # np has a built-in function to sample from a multinomial distribution
...
def Q_learning_step(Q_function, state, action, reward, next_state, next_action, terminal, alpha, gamma):
    ...
    # YOUR IMPLEMENTATION HERE #
    ############################
    next_Q_function = Q_function.copy()
    if not terminal:
        next_Q_function[state][action] = Q_function[state][action] + alpha * (reward + gamma * np.max(Q_function[next_state]) - Q_function[state][action])
    else:
        # handle terminal condition
        next_Q_function[state][action] = Q_function[state][action] + alpha * (reward - Q_function[state][action])
...
def Sarsa_step(Q_function, state, action, reward, next_state, next_action, terminal, alpha, gamma):
    ...
    next_Q_function = Q_function.copy()
    if not terminal:
        next_Q_function[state][action] = Q_function[state][action] + alpha * (reward + gamma * Q_function[next_state][next_action] - Q_function[state][action])
    else:
        # if terminal, Q_function[next_state] = 0
        next_Q_function[state][action] = Q_function[state][action] + alpha * (reward - Q_function[state][action])
